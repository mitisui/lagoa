...
// MIGRAÇÃO: net.minecraftforge.event -> net.neoforged.neoforge.event
...
public class SwordEvents {
    ...
    private static final SoundEvent BEACON_SOUND = SoundEvent.createVariableRangeEvent(
            ResourceLocation.fromNamespaceAndPath("minecraft", "beacon_massive_attack")
            // MIGRAÇÃO: new ResourceLocation("minecraft", "x") -> ResourceLocation.fromNamespaceAndPath("minecraft", "x")
    );
    ...
    public static void onSwordSpecial(PlayerInteractEvent.RightClickItem event) {
        ...
        boolean temNome = item.has(DataComponents.CUSTOM_NAME)
                && item.getHoverName().getString().contains("Divisor de Almas");
    ...
    private static void performSoulDivision(Player attacker, LivingEntity target, Level level) {
        ...
        // MIGRAÇÃO: Thread.sleep em server.execute é perigoso — usar schedule via tickCount
        // Armazena o target para prender após 20 ticks (1 segundo)
        PENDING_PRISON.put(target.getUUID(), level.getGameTime() + 20L);
    }

    // Mapa para delay da prisão (evita Thread.sleep que era perigoso)
    private static final Map<UUID, Long> PENDING_PRISON = new HashMap<>();
    ...
    private static void imprisonTarget(LivingEntity target, Level level) {
        ...
        IMPRISONED_PLAYERS.put(target.getUUID(),
                new PrisonData(prisonCenter, currentTime, currentTime + duracaoPrisao));
        ...
        level.playSound(null, target.blockPosition(), SoundEvents.WITHER_AMBIENT,   SoundSource.HOSTILE, 2.0F, 0.5F);
        level.playSound(null, target.blockPosition(), SoundEvents.ENDER_DRAGON_GROWL, SoundSource.HOSTILE, 1.5F, 0.8F);
    }
    ...
    private static void createSoulPrison(Vec3 center, ServerLevel level) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (double y = 0; y <= radius * 2; y += 0.3) {
                level.sendParticles(ParticleTypes.SMOKE, x, center.y - radius + y, z, 1, 0, 0, 0, 0);
            }
        }
    }

    // MIGRAÇÃO: TickEvent.LevelTickEvent (Forge) -> LevelTickEvent.Post (NeoForge)
    @SubscribeEvent
    public static void onWorldTick(LevelTickEvent.Post event) {
        ...
        // Processa prisoners pendentes (delay substituindo o Thread.sleep)
        PENDING_PRISON.entrySet().removeIf(entry -> {
            if (currentTime >= entry.getValue()) {
                LivingEntity target = (LivingEntity) serverLevel.getEntity(entry.getKey());
                if (target != null) imprisonTarget(target, serverLevel);
                return true;
            }
            return false;
        });

        // Ticks dos prisioneiros
        IMPRISONED_PLAYERS.entrySet().removeIf(entry -> {
            ...
            if (currentTime >= data.endTime) return true;
            ...
            if (data.damageCounter >= data.damageInterval) {
                prisoner.hurt(serverLevel.damageSources().magic(), 2.0F);
            ...
            if (currentTime % 5 == 0) createSoulPrison(data.prisonCenter, serverLevel);

            return false;
        });

        // Remove escudo temporário
    ...
    private static void performMassiveAttack(Player player, Level level) {
        ...
        level.getEntities(player, area).forEach(entity -> {
            if (entity instanceof Player target) {
                ...
                target.addEffect(new MobEffectInstance(MobEffects.DARKNESS, 400, 4, false, true, true));
                target.addEffect(new MobEffectInstance(MobEffects.UNLUCK,  1200, 0, false, true, true));

                ((ServerLevel)level).sendParticles(ParticleTypes.FLASH,
                        target.getX(), target.getY() + 1, target.getZ(), 5, 0.2, 0.2, 0.2, 0.0);
        ...
        for (int y = bedrockLevel; y <= worldHeight; y += 2) {
            serverLevel.sendParticles(ParticleTypes.END_ROD,
                    player.getX(), y, player.getZ(), 8, 0.1, 0.1, 0.1, 0.01);

            double scale = 1.5;
            for (int i = 0; i < 4; i++) {
                double angle = (Math.PI * i) / 2;
                serverLevel.sendParticles(ParticleTypes.SOUL_FIRE_FLAME,
                        player.getX() + Math.cos(angle) * scale, y,
                        player.getZ() + Math.sin(angle) * scale,
                        2, 0.2, 0.2, 0.2, 0.02);
            }

            if (y % 4 == 0) {
                serverLevel.sendParticles(ParticleTypes.ENCHANT,
                        player.getX(), y, player.getZ(), 12, scale, 0.5, scale, 0.05);
            }
        }
        ...
        for (int i = 0; i < 360; i += 10) {
            ...
            serverLevel.sendParticles(ParticleTypes.FLASH,
                    player.getX() + Math.cos(angle) * radius, player.getY(),
                    player.getZ() + Math.sin(angle) * radius,
                    1, 0, 0, 0, 0);
        }

        level.playSound(null, player.blockPosition(), BEACON_SOUND,   SoundSource.PLAYERS, 3.0F, 0.8F);
        level.playSound(null, player.blockPosition(), SoundEvents.WITHER_SPAWN, SoundSource.HOSTILE, 2.0F, 0.5F);
    }
    ...
    private static void replaceBlockWithBlackConcrete(Level level, BlockPos pos) {
        ...
        if (currentState.isAir() || currentBlock == Blocks.BEDROCK ||
                currentBlock == Blocks.BARRIER || currentBlock == Blocks.COMMAND_BLOCK) return;
        level.setBlock(pos, Blocks.BLACK_CONCRETE.defaultBlockState(), 3);
    }

    // MIGRAÇÃO: LivingAttackEvent -> LivingIncomingDamageEvent (NeoForge 1.21.1)
    @SubscribeEvent
    public static void onProjectileHit(LivingIncomingDamageEvent event) {
        ...
        if (!mainHand.has(DataComponents.CUSTOM_NAME) ||
                !mainHand.getHoverName().getString().contains("Divisor de Almas")) return;
